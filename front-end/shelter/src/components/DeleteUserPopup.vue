<template>
    <v-dialog v-model="dialog" max-width="60vw">
        <template v-slot:activator="{ props }">
            ...
            <v-btn elevation="8" class="text font-big" v-bind="props"> <v-icon color="rgb(143, 83, 122)"
                    icon="mdi-delete-empty"></v-icon>Usuń konto</v-btn>
        </template>


        <div class="d-flex align-center flex-column" style="width: 50vw; height: fit-content; margin: auto;">
            <v-sheet style="
                float: left;
                width: 100%;
                border-radius: 60px;
              ...
              ">
                ...
                <div style="padding: 10px; margin: auto; width: fit-content; text-align: center;">
                    ...
                    font-size: x-large;
                  " class="font">
                        Uwaga! <br> usunięcie konta trwałe! Czy na pewno chcesz usunąć? <br>


                    </h3>
                </div>
                <div style="width: fit-content; margin: 0 auto;">
                    <v-btn class="little-title" @click="logout(), dialog = false, $router.push('/') ">USUŃ</v-btn>
                    <v-divider vertical :thickness="20" class="border-opacity-0"></v-divider>
                    <v-btn class="little-title" @click="dialog = false">Zrezygnuj</v-btn>
                </div>
...
async function logout() {
    await userStore.deleteUser(userStore.userData.email)
    userStore.$reset()
    animalsStore.$reset()
}
